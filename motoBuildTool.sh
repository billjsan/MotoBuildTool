...
motoBuildTool() {
  ...
  FEATURES=("Make an apk" "generate a build" "$EXIT")
  ...
  echo -e "⚠️ ${RED} This version was only tested on qcom builds yet ${NC}⚠️"
  ...
  echo -e "🧠 ${YELLOW_BOLD}What do you want to do?${NC}"
  select FEATURE in "${FEATURES[@]}"; do
      [[ $FEATURE == "$EXIT" ]] && echo -e "🚫 ${RED}Cancelled.${NC}" && return
      [[ -n $FEATURE ]] && break
      echo -e "🚫 ${RED} Invalid option.${NC}"
  done

  if [[ "$FEATURE" == "Make an apk" ]]; then
        # MAKE APK
        echo -e "📱 ${YELLOW_BOLD}Select a device:${NC}"
          ...
          select APK in "CarrierSettingsExt" "Settings" "TMOConfig" "TrafficStatsProvider" "TrafficStatsTests" "exit"; do
            [[ $APK == "exit" ]] && echo "Cancelled." && return
            [[ $APK == "Settings" || $APK == "TrafficStatsProvider" || $APK == "TrafficStatsTests" || $APK == "CarrierSettingsExt" || $APK == "TMOConfig" ]] && break
            echo -e "🚫 ${RED} Invalid option.${NC}"
          done
          ...
          LOG_FILE="$SCRIPT_DIR/out/log/build_${DEVICE}_${APK}_${TIMESTAMP}.log"
          ...
          ssh "$SSH_HOST" bash <<EOF > "$LOG_FILE" 2>&1
            ...
            if [[ "$INCLUDE_PATCH" == "Y" || "$INCLUDE_PATCH" == "y" ]]; then
              ...
              case "$APK" in
                "TMOConfig")
                  cd motorola/cbs/tmo/packages/apps/TMOConfig
                ;;
                "CarrierSettingsExt")
                  cd motorola/packages/apps/CarrierSettingsExt
                ;;
                "Settings")
